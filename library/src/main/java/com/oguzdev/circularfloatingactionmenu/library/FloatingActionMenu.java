...
public class FloatingActionMenu {
	...
	public void open(boolean animated) {
		// Cancel previous auto close runnable
		if (null != autoCloseRunnable) {
			autoCloseRunnable.cancel();
		}
		...
		// Auto close, default 3 seconds
		if (autoClose) {
			autoCloseRunnable = new AutoCloseRunnable();
			mainActionView.postDelayed(autoCloseRunnable, autoCloseTimeout);
		}
	}

	private boolean autoClose = true;
	private int autoCloseTimeout = 3000;
	...
	private AutoCloseRunnable autoCloseRunnable;

	class AutoCloseRunnable implements Runnable {
	...
	private Point getActionViewCoordinates() {
		...
		coords[0] -= (getScreenSize().x - getActivityContentView()
				.getMeasuredWidth());
		coords[1] -= (getScreenSize().y - getActivityContentView()
				.getMeasuredHeight());
